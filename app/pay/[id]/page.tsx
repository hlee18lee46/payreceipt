...
export default function CheckoutPage() {
  ...
  // 2. Polling for NFT status (Fallback if user refreshes before mint completes)
useEffect(() => {
    ...
    interval = setInterval(async () => {
      try {
        ...
        if (data.ok && data.invoice?.nftExplorer) {
          setInvoice(data.invoice);
          clearInterval(interval); // Stop polling once we have the link
        }
      } catch (err) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (hash) {
        setTxId(hash) // Immediately show success screen
...
if (data.ok) {
  setInvoice((prev: any) => {
    // 1. Extract the explorer link from any possible key
    const rawExplorer = data.explorer || data.nftExplorer || data.invoice?.nftExplorer;
    
    // 2. FALLBACK: If we only got a transaction hash, build the full Nile URL manually
    const finalExplorer = rawExplorer?.includes("http") 
      ? rawExplorer 
      : rawExplorer ? `https://nile.tronscan.org/#/transaction/${rawExplorer}` : null;
    ...
    return {
      ...prev,
      ...data.invoice, // Merge in all fresh database fields
      status: 'paid',
      nftExplorer: finalExplorer, // This unlocks the button text
      imageUrl: finalImage,       // This flips the preview from Loader to Image
    };
  });
          ...
          <Card className="max-w-md w-full text-center border-green-500/20 shadow-2xl overflow-hidden">
            ...
            <CardContent className="pt-10 pb-10 space-y-6">
              ...
              <div className="space-y-2">
                <CardTitle className="text-2xl font-bold tracking-tight">Payment Successful</CardTitle>
                <p className="text-muted-foreground text-sm px-4">
                  Your payment of <span className="font-bold text-foreground">{invoice.amount} TRX</span> is confirmed. Your digital receipt NFT has been issued.
                </p>
              </div>

              {/* Digital Asset Preview with Loading State */}
              <div className="mt-4 mb-2 px-4 animate-in fade-in zoom-in duration-500">
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-2 text-left">Digital Asset Preview</p>
                <div className="rounded-lg border overflow-hidden shadow-sm bg-white ring-1 ring-black/5 min-h-[200px] flex items-center justify-center relative">
                  {invoice.imageUrl ? (
                    <img 
                    ...
                    <div className="flex flex-col items-center gap-3 py-12">
                      ...
                      <p className="text-xs text-neutral-400 font-medium animate-pulse">Minting NFT Receipt...</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid gap-3 pt-2">
                {/* Payment Link */}
                <Button asChild variant="outline" className="w-full py-6 font-mono text-[10px] text-neutral-500">
                ...
                >
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    onClick={(e) => { if (!invoice.nftExplorer) e.preventDefault(); }}
                  >
